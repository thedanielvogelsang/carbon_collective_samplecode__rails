class Api::V1::Areas::RegionElectricityController < ApplicationController
  ...
  def index
    if params[:country]
      ...
      render json: Region.where(country_id: id).joins(:users).order(:total_electricity_saved).distinct
    else
      render json: Region.order(avg_daily_electricity_consumed_per_capita: :asc)
    end
  end
