class Api::V1::Areas::NeighborhoodElectricityController < ApplicationController
  def index
    if params[:city]
      ...
      render json: Neighborhood.where(city_id: id).joins(:users).order(total_electricity_saved: :desc).distinct
    else
      render json: Neighborhood.joins(:users).order(total_electricity_saved: :desc).distinct
    end
  end
