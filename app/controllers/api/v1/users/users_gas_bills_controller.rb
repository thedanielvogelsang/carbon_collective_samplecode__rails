class Api::V1::Users::UsersGasBillsController < ApplicationController
  ...
  def index
    if params[:user_id] && User.friendly.exists?(params[:user_id])
      ...
      render json: HeatBill.joins(:house)
                ...
                .order(end_date: :desc)
                .select{|b| b.start_date >= (uh.move_in_date - (60*60*24))}, each_serializer: HeatBillSerializer
    end
  end
  ...
  private
    def safe_params
      params.require(:gas_bills).permit(:start_date, :end_date, :total_therms, :price, :house_id, :no_residents, :who, :force)
    end
end
