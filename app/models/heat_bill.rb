class HeatBill < ApplicationRecord
 ...
 def subtract_from_users_totals
   ...
   users.each do |u|
     ...
     u.total_therms_logged.to_i == 0 ? clear_user_ranks(u) : nil
 ...
 def clear_user_ranks(user)
   user.user_gas_rankings.each do |uwr|
 ...
 def detect_outlier
   ...
   return average_daily_usage > 0 && average_daily_usage <= max ? true : force ? true : errors.add(:total_therms, "resource usage is much higher than average, are you sure you want to proceed?")
 end
