module CountryHelper
  ...
  def update_daily_avg_water_savings
    water_savings = self.users.map{|u| u.total_water_savings }.flatten.reject(&:nan?)
    .reduce(0){|sum, num| sum + num } / self.users.count if self.users.count != 0
  ...
  def update_total_gas_savings
    gas_saved = self.users.map{|u| u.total_gas_savings}.flatten.reject(&:nan?)
              .reduce(0){|sum, num| sum + num}
    self.total_gas_saved = gas_saved
  end

  def update_daily_avg_gas_savings
    gas_savings = self.users.map{|u| u.total_gas_savings }.flatten.reject(&:nan?)
            .reduce(0){|sum, num| sum + num } / self.users.count if self.users.count != 0
    self.avg_total_gas_saved_per_user = gas_savings
  end

  def update_daily_avg_gas_consumption
    gas_consumed = self.users.map{|u| u.avg_daily_gas_consumption }.flatten.reject(&:nan?)
    .reduce(0){|sum, num| sum + num} / self.users.count if self.users.count != 0
