...
RSpec.describe Region, type: :model do
  ...
  context 'methods/ attributes' do
    it 'automatically capitalizes name' do
      region = Region.create(name: "name", country_id: @country.id)
        expect(region.name).to eq "Name"
    end
    it 'initializes with zeros for averages' do
      region = Region.create(name: "name", country_id: @country.id)
        expect(region.total_electricity_saved).to eq 0
        expect(region.total_water_saved).to eq 0
        expect(region.total_gas_saved).to eq 0
        expect(region.avg_total_electricity_saved_per_user).to eq 0
        expect(region.avg_total_water_saved_per_user).to eq 0
        expect(region.avg_total_gas_saved_per_user).to eq 0
        expect(region.avg_daily_electricity_consumed_per_user).to eq 0
        expect(region.avg_daily_water_consumed_per_user).to eq 0
        expect(region.avg_daily_gas_consumed_per_user).to eq 0
        #per capitas are left null
          expect(region.avg_daily_electricity_consumed_per_capita).to eq nil
          expect(region.avg_daily_water_consumed_per_capita).to eq nil
          expect(region.avg_daily_gas_consumed_per_capita).to eq nil
    end
  end
  context 'validations/ associations' do
    it {should validate_presence_of(:name)}
    it {should validate_uniqueness_of(:name).scoped_to(:country_id).case_insensitive}
    ...
    it {should have_many(:region_snapshots)}
