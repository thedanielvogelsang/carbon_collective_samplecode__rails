...
RSpec.describe Neighborhood, type: :model do
  ...
  context 'methods/ attributes' do
    it 'automatically capitalizes name' do
      neighborhood = Neighborhood.create(name: "name", city_id: @city.id)
        expect(neighborhood.name).to eq "Name"
    end
    it 'initializes with zeros for averages' do
      neighborhood = Neighborhood.create(name: "name", city_id: @city.id)
        expect(neighborhood.total_electricity_saved).to eq 0
        expect(neighborhood.total_water_saved).to eq 0
        expect(neighborhood.total_gas_saved).to eq 0
        expect(neighborhood.avg_total_electricity_saved_per_user).to eq 0
        expect(neighborhood.avg_total_water_saved_per_user).to eq 0
        expect(neighborhood.avg_total_gas_saved_per_user).to eq 0
        expect(neighborhood.avg_daily_electricity_consumed_per_user).to eq 0
        expect(neighborhood.avg_daily_water_consumed_per_user).to eq 0
        expect(neighborhood.avg_daily_gas_consumed_per_user).to eq 0
        expect(neighborhood.avg_daily_electricity_consumed_per_capita).to eq 0
        expect(neighborhood.avg_daily_water_consumed_per_capita).to eq 0
        expect(neighborhood.avg_daily_gas_consumed_per_capita).to eq 0
    end
  end
  context 'validations/ associations' do
    it {should validate_presence_of(:name)}
    it {should validate_uniqueness_of(:name).scoped_to(:city_id).case_insensitive}
    it {should belong_to(:city)}
    it {should have_many(:addresses)}
    it {should have_many(:houses)}
    it {should have_many(:users)}
    it {should have_many(:neighborhood_snapshots)}
