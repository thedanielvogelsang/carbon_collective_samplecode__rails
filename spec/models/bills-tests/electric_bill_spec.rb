...
RSpec.describe ElectricBill, type: :model do
  ...
  context 'in a house' do
    it 'cant have bills with the same exact days' do
      ...
      el2 = ElectricBill.new(total_kwhs: 1000,  start_date: yesterday, end_date: (yesterday + 29), house_id: @house.id, no_residents: 2, who: @user)
      expect(el1.save).to be true
    ...
    it 'bill B cant save if start_date before the end_date of bill A' do
      yesterday = DateTime.now - 29
      bill_A = ElectricBill.new(total_kwhs: 1000, start_date: yesterday, end_date: (yesterday + 11), house_id: @house.id, no_residents: 2, who: @user)
      bill_B = ElectricBill.new(total_kwhs: 1000,  start_date: yesterday + 10, end_date: (yesterday + 29), house_id: @house.id, no_residents: 2, who: @user)
    ...
    it 'bill B cant save if end_date after the start_date of bill A' do
      ...
      ago = DateTime.now - 29
      bill_A = ElectricBill.new(total_kwhs: 1000, start_date: yesterday, end_date: (yesterday + 20), house_id: @house.id, no_residents: 2, who: @user)
      bill_B = ElectricBill.new(total_kwhs: 1000,  start_date: ago, end_date: (yesterday + 1), house_id: @house.id, no_residents: 2, who: @user)
      expect(bill_A.save).to be true
      expect(bill_B.save).to be false
      expect(bill_B.errors.messages).to eq({:start_date=>["start or end date overlaps with another bill"]})
    end
    it 'bill B CAN save if BOTH start_date and end_date BEFORE bill A' do
      yesterday = DateTime.now - 20
      ago = DateTime.now - 29
      bill_A = ElectricBill.new(total_kwhs: 1000, start_date: yesterday, end_date: (yesterday + 20), house_id: @house.id, no_residents: 2, who: @user)
      bill_B = ElectricBill.new(total_kwhs: 1000,  start_date: ago, end_date: (yesterday - 1), house_id: @house.id, no_residents: 2, who: @user)
    ...
    it 'bill B CAN save if BOTH start_date and end_date AFTER bill A' do
      yesterday = DateTime.now - 20
      ago = DateTime.now - 29
      bill_A = ElectricBill.new(total_kwhs: 1000, start_date: ago, end_date: (yesterday - 1), house_id: @house.id, no_residents: 2, who: @user)
      bill_B = ElectricBill.new(total_kwhs: 1000,  start_date: yesterday, end_date: (yesterday + 20), house_id: @house.id, no_residents: 2, who: @user)
      expect(bill_A.save).to be true
      expect(bill_B.save).to be true
    end
  end
  context 'in a house with revolving users' do
    ...
    it 'bill saves update users whos move in dates precede bill date' do
      ...
      new_user_score = new_user.avg_daily_electricity_consumption
      old_user_score = old_user.avg_daily_electricity_consumption
      oldest_user_score = oldest_user.avg_daily_electricity_consumption
      ...
      new_user_score_NEW = User.find(new_user.id).avg_daily_electricity_consumption
      old_user_score_NEW = User.find(old_user.id).avg_daily_electricity_consumption
      oldest_user_score_NEW = User.find(oldest_user.id).avg_daily_electricity_consumption
