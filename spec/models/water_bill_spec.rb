...
RSpec.describe WaterBill, type: :model do
  ...
  context 'a house' do
    it 'cant have bills with the same exact days' do
      today = DateTime.now
      el1 = WaterBill.new(total_gallons: 1000, start_date: today, end_date: (today + 30), house_id: @house.id)
      el2 = WaterBill.new(total_gallons: 1000,  start_date: today, end_date: (today + 30), house_id: @house.id)
      expect(el1.save).to be true
      expect(el2.save).to be false
    end
    it 'bill B cant save if start_date before the end_date of bill A' do
      today = DateTime.now
      bill_A = WaterBill.new(total_gallons: 1000, start_date: today, end_date: (today + 30), house_id: @house.id)
      bill_B = WaterBill.new(total_gallons: 1000,  start_date: today + 29, end_date: (today + 40), house_id: @house.id)
    ...
    it 'bill B cant save if end_date after the start_date of bill A' do
      ...
      ago = DateTime.now - 30
      bill_A = WaterBill.new(total_gallons: 1000, start_date: today, end_date: (today + 30), house_id: @house.id)
      bill_B = WaterBill.new(total_gallons: 1000,  start_date: ago, end_date: (today + 1), house_id: @house.id)
      expect(bill_A.save).to be true
      expect(bill_B.save).to be false
    end
    ...
    it 'bill B CAN save if start_date and end_date AFTER bill A' do
      today = DateTime.now
      ago = DateTime.now - 30
      bill_A = WaterBill.new(total_gallons: 1000, start_date: ago, end_date: (today - 1), house_id: @house.id)
      bill_B = WaterBill.new(total_gallons: 1000,  start_date: today, end_date: (today + 20), house_id: @house.id)
      expect(bill_A.save).to be true
      expect(bill_B.save).to be true
    end
  end
